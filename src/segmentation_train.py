...
def train_segmentation_model(
    ...
    experiment.config.update(
        dict(epochs=epochs, batch_size=batch_size, learning_rate=learning_rate, weight_decay=weight_decay,
             val_percent=val_percent, class_weights=class_weights, save_checkpoint=save_checkpoint, img_dim=img_dim, model=model.__class__.__name__, amp=amp, optimizer=optimizer, dropout=0)
    )
    ...
    logging.info(f"Test evaluation complete.")
